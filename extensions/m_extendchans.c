 ...
 *
 *      Imported from ircd-seven and adapted to work with ChatIRCd.  Added
 *  the ability for opers who are NetAdmins or possess the oper:extendchans
 *  flag to increase the channel limit of a connected user.  Only remains in
 *  effect for the duration of the user session.
 *
...
mo_extendchans(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	if(!IsNetAdmin(source_p) && !HasPrivilege(source_p, "oper:extendchans"))
	{
