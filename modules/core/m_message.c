	    ...
	    const char *text)
{
	...
	if((result = can_send(chptr, source_p, NULL)))
	{
		...
		{
			if (p_or_n != PRIVMSG && chptr->mode.mode & MODE_NONOTICE && !IsService(source_p))
			{
	   ...
	   struct Client *source_p, struct Client *target_p, const char *text)
{
	...
	if(MyClient(target_p))
	{
		...
		if(!IsServer(source_p) && (IsSetCallerId(target_p) || (IsSetRegOnlyMsg(target_p) && !source_p->user->suser[0]) ||
